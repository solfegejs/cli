...
prettyError.skip(traceLine => {
  ...
  if (typeof traceLine.path === "string" && traceLine.path.indexOf("internal/") === 0) {
    return true;
  }
  return false;
});
...
module.exports = class Bundle {
  ...
  displayError(error) {
    ...
    const wrappedMessage = wrap(String(error.message), {
      ...
      escape: row => {
        let newRow = "│ ";
        newRow += row;
        if (row.length < availableWidth) {
          newRow += " ".repeat(availableWidth - row.length);
        }
        newRow += " │";
        return newRow;
      }
